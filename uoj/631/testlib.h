...
NORETURN void InStream::quit(TResult result, const char* msg)
{
	...
	if (localJudger)
	{
		...
		else if (result == _points) exit((int)(100+partialScore));
		exit(1);
	}
...
void registerLocalChecker(const std::string &_inputFile, const std::string &_outputFile, const std::string &_answerFile, int _perfectScore, const std::string &_reportFile = "")
{
	localJudger = 1;
	__testlib_ensuresPreconditions();
...
{
	registerLocalChecker(argv[1], argv[3], argv[2], 100, argv[4]);
}
