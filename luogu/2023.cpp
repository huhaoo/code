/****************************************************************
	...
	Create time: 10/28/2018, 12:12:10 PM
****************************************************************/
#include<stdio.h>
#include<string.h>
#include<math.h>
	...
	while(q--)
	{
		...
		if(opt==1)
		{
			...
			if(L==R)
			{
				...
				fr(i,l,r)s[L]=(s[L]+(v[i]=(v[i]*l1[L]+l2[L])%p*c%p))%p;
				l1[L]=1;l2[L]=0;
				continue;
			}
			fr(i,L+1,R-1)
			...
			s[L]=s[R]=0;
			fr(i,L*b-b+1,l-1)s[L]=(s[L]+(v[i]=(v[i]*l1[L]+l2[L])%p))%p;
			fr(i,l,L*b)s[L]=(s[L]+(v[i]=(v[i]*l1[L]+l2[L])%p*c%p))%p;
			fr(i,R*b-b+1,r)s[R]=(s[R]+(v[i]=(v[i]*l1[R]+l2[R])%p*c%p))%p;
