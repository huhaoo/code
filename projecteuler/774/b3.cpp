...
i64 dp2(int n,int m,int c1,int c2,int c0,int o)
{
	...
	if(!v2[id[m][c1][c2][c0]][o])
	{
		...
		fr(j,m,l) if(Q(j,j))
		{
			...
			static i64 S[L][L][L];
			fr(x,0,j) fr(y,0,j) fr(z,0,j) S[x][y][z]=0;
			fr(v0,0,C0) fr(v1,0,C1) fr(v2,0,C2) fr(v3,0,C3)
				S[v0+v1+v2+v3][v0+v1][v0+v2]=(S[v0+v1+v2+v3][v0+v1][v0+v2]+CC[C0][v0][C1][v1]*CC[C2][v2][C3][v3])%mod;
			fr(x,0,j) fr(y,0,j) fr(z,0,j) fr(_n,3,::n) fr(i,2,_n-1)
			{
				i64 _s=0;
				fr(v0,0,C0) fr(v1,0,C1) fr(v2,0,C2) fr(v3,0,C3)
					if(x==v0+v1+v2+v3&&y==v0+v1&&z==v0+v2) _s=(_s+CC[C0][v0][C1][v1]*CC[C2][v2][C3][v3])%mod;
				if(_s!=S[x][y][z]) printf("%d %d %d  %d %d %d %d  %lld %lld\n",x,y,z,C0,C1,C2,C3,_s,S[x][y][z]);
				f2[_n][id[m][c1][c2][c0]][o]=(f2[_n][id[m][c1][c2][c0]][o]+_s*
				dp2(i,j,C0+C1,x,y,2+(j==m&&(o&1)))%mod*
				dp2(_n-i+1,j+1,C0+C2+(j!=m||!(o&2)),x,z,0))%mod;
	//			if(flag) printf(" %d %d  %d %d %d %d   %d %d %d %d  %lld\n",i,j,C0,C1,C2,C3,v0,v1,v2,v3,ans);
			}
			continue;
//			fr(v0,0,C0) fr(v1,0,C1) fr(v2,0,C2) fr(v3,0,C3)
			fr(x,0,j) fr(y,0,j) fr(z,0,j) if(S[x][y][z]) fr(_n,3,::n) fr(i,2,_n-1)
			{
				f2[_n][id[m][c1][c2][c0]][o]=(f2[_n][id[m][c1][c2][c0]][o]+S[x][y][z]*
				dp2(i,j,C0+C1,x,y,2+(j==m&&(o&1)))%mod*
				dp2(_n-i+1,j+1,C0+C2+(j!=m||!(o&2)),x,z,0))%mod;
			}
		}
	}
//	if(f2[n][id[m][c1][c2][c0]][o]) printf("%d %d %d %d %d %d   %d\n",n,m,c1,c2,c0,o,f2[n][id[m][c1][c2][c0]][o]);
	return f2[n][id[m][c1][c2][c0]][o];
}
...
int main()
{
	n=30; fr(i,0,n) fr(j,0,i) fr(k,0,i) fr(o,0,std::min(j,k)) id[i][j][k][o]=++Id;
	fr(i,0,n){ C[i][0]=C[i][i]=1; fr(j,1,i-1) C[i][j]=(C[i-1][j]+C[i-1][j-1])%mod; }
